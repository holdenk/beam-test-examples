...
public class DebuggingWordCount {
  ...
  public interface WordCountOptions extends WordCount.WordCountOptions {
    ...
    @Default.String("Flourish|stomach|panda")
  ...
  public static void main(String[] args) {
    ...
    // Validate that we haven't thrown away most things.
    // Note: not all runners support commited, instead attempted may be available
    // with more spark-like-semantics. Note: this is applied after counting.
    Long matchedWordsValue = metrics.queryMetrics(MetricsFilter.builder()
        .addNameFilter(MetricNameFilter.named(FilterTextFn.class, "matchedWords")).build())
        .counters().iterator().next().committed();

    Long unmatchedWordsValue = metrics.queryMetrics(MetricsFilter.builder()
        .addNameFilter(MetricNameFilter.named(FilterTextFn.class, "unmatchedWords")).build())
        .counters().iterator().next().committed();
